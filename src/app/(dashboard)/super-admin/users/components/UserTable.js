...
import { 
  ChevronUp, ChevronDown, Edit, Trash2, User, Mail, 
  Loader2, Key, Eye, UserCheck, UserX, UserCog, Shield, Briefcase, Search 
} from 'lucide-react';
import Pagination from '@/components/common/Pagination';
import UserFilters from './UserFilters';
import { userManagementService } from '@/services/userManagementService';
...
export default function UserTable() {
    ...
    try {
      ...
      if (usersResponse.success) {
        const transformedData = usersResponse.data.map(user => ({
          ...
          companyRole: user.companyRole?.displayName || 'No role assigned',
          status: user.isActive ? 'Active' : 'Inactive',
          createdAt: new Date(user.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
          isActive: user.isActive,
          hasEmployee: !!user.employee,
          employee: user.employee,
          companyRoleId: user.companyRoleId
  ...
  useEffect(() => { fetchData(); }, [pagination.pageIndex, pagination.pageSize, globalFilter, statusFilter, roleFilter]);
  ...
  const roleOptions = useMemo(() => {
    const roles = ['all'];
    const roleMap = {};
    systemRoles.forEach(r => { if (!roleMap[r.name]) { roleMap[r.name] = true; roles.push(r.displayName || r.name); }});
    availableRoles.forEach(r => { if (!roleMap[r.name]) { roleMap[r.name] = true; roles.push(r.displayName || r.name); }});
    return roles;
  }, [availableRoles, systemRoles]);
  ...
  const columns = useMemo(() => [
    {
      ...
      cell: info => (
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-semibold text-sm shadow-md">
        ...
        return (
          <div className="flex items-center">
             <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-medium border ${
               ...
               {isSystem ? <Shield size={10} /> : <Briefcase size={10} />}
               {roleDisplay}
             </span>
          </div>
        );
      ...
      cell: info => {
        ...
        return (
          <div className="flex items-center gap-2 justify-end">
            <button
              onClick={() => handleAssignRoleClick(user)}
              className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-blue-600 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors tooltip-trigger"
              title="Assign Role"
            >
              <UserCog size={16} />
            </button>
            ...
            <button
              onClick={() => handleResetPassword(user)}
              className="p-2 rounded-lg text-gray-500 hover:bg-yellow-50 hover:text-yellow-600 dark:text-gray-400 dark:hover:bg-yellow-900/20 transition-colors"
              title="Reset Password"
            >
              <Key size={16} />
            </button>
  ...
  const clearFilters = () => { setStatusFilter('all'); setRoleFilter('all'); setGlobalFilter(''); };

  const table = useReactTable({
    data, columns, state: { sorting, globalFilter, pagination },
  ...
  return (
    <div className="space-y-6">
      {loading && (
        <div className="fixed inset-0 bg-white/50 dark:bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-xl shadow-xl flex items-center gap-3">
             ...
             <span className="font-medium text-gray-700">Loading Users...</span>
          </div>
        </div>
      )}
      ...
      <UserFilters
        ...
        statuses={['all', 'Active', 'Inactive']} roles={roleOptions}
        onClearFilters={clearFilters}
      />
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-gray-700">
            ...
            <div className="mb-6">
              ...
              <select
                value={selectedRoleForAssign}
                onChange={(e) => setSelectedRoleForAssign(e.target.value)}
                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
              >
                <option value="">Select a role...</option>
                <optgroup label="System Roles">
                  {systemRoles.map(role => (
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={handleAssignRoleSubmit}
                className="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors shadow-lg shadow-blue-600/20"
              >
                Confirm Assignment
              </button>
            </div>
