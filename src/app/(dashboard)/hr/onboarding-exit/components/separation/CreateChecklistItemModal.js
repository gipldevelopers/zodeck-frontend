...
export default function CreateChecklistItemModal({ isOpen, onClose, resignationId, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onboardingExitService.createSeparationChecklistItem(resignationId, payload);
      toast.success("Checklist item created successfully");
      onSuccess();
      setFormData({ title: "", description: "", category: "OTHER", dueDate: "", assignedTo: "" });
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Create Checklist Item</h2>
          <button
            onClick={onClose}
