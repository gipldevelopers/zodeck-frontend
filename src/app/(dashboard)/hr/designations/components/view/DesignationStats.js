...
export default function DesignationStats({ designation }) {
  const [animatedCount, setAnimatedCount] = useState(0);

  useEffect(() => {
    if (designation && designation.employeeCount > 0) {
      // Animate the employee count
      let start = 0;
      const end = designation.employeeCount;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-fadeIn">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <div className="flex items-center">
          ...
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400">Experience Range</p>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{designation.level}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{designation.status}</p>
          </div>
        </div>
      </div>
    </div>
